 ...
 * <UL>
 *   <LI>
 *     setServerIP(String serverIP):  Sets the TFTP Server IP.  This method does
 *     not return anything.
 *   </LI>
 *   <LI>
 *     setFileName(String fileName):  Assigns the file name to download from the
 *     TFTP server.  This method does not return anything.
 *   </LI>
 *   <LI>
 *     setDataTransferMode(String mode):    Indicates how the data should be
 *     transferred ie Binary, ASCII, and mail.  This method does not return
 *     anything.
 *   </LI>
 *   <LI>
 *     setNumberOfRetries(int numberOfRetries):  Affects how many times the
 *     client will try to retransmit the current packet.  This method does not
 *     return anything.
 *   </LI>
 *   <LI>
 *     setTimeout(int timeout):  Sets the timeout, in milliseconds, to wait for
 *     a packet to arrive over the network.  This method does not return
 *     anything.
 *   </LI>
 *   <LI>
 *     getServerIP():  Returns the server IP as a string.  The default is
 *     <I>"127.0.0.1"</I>.
 *   </LI>
 *   <LI>
 *     getFileName():  Returns a string containing the file name to fetch.  The
 *     default is <I>"filename"</I>.
 *   </LI>
 *   <LI>
 *     getDataTransferMode():  Returns the current transfer mode stored as a
 *     string.  The default is <I>"octet"</I>.
 *   </LI>
 *   <LI>
 *     getNumberOfRetries():  Returns the number of retries as an integer.  The
 *     default is <I>3</I>.
 *   </LI>
 *   <LI>
 *     getTimeout():  Returns an integer with the timeout, in    milliseconds.
 *     The default is <I>3000</I> which is equivalent to 3 seconds.
 *   </LI>
 *   <LI>
 *     getErrorMessage():  When an error message is received by the client, the
 ...
 *   </LI>
 *   <LI>
 *     getFetchedDataLength():  Returns the total length of the downloaded
 *     file.  It returns 0 if nothing has been fetched yet.
 *   </LI>
 *   <LI>
 *     getFile():  Fetches a file from the TFTP server.  Returns 0 if everything
 *     worked fine, 1 if an error packet was received, 2 if the maximum number
 *     of retries has been exceeded, and 3 if a TFTPClientException has been
 *     thrown by the TFTP client.
 *   </LI>
 *   <LI>
 *     enableStatisticsCollection():  Enables the gathering of statistical
 *     information.
 *   </LI>
 *   <LI>
 *     disableStatisticsCollection():  Disables the gathering of statistical
 *     information.
 *   </LI>
 *   <LI>
 *     isStatisticsEnabled():  Returns true if statistics collection is enabled
 *     and false if not.
 *   </LI>
 *   <LI>
 *     areWeCollectingNow():  Returns true if the client is currently collecting
 *     statistical information and false if not.
 *   </LI>
 * </UL>
 *
...
public class TFTPClientVariable extends Variable
{
  ...
  /**
   * The name of the method that is used to know the, currently set, TFTP
   * server IP.
   */
  ...
  public static final Method[] TFTP_CLIENT_VARIABLE_METHODS = new Method[]
       {
            ...
            new Method(ENABLE_STATISTICS_COLLECTION_METHOD_NAME,
                 new String[0], null),
            new Method(DISABLE_STATISTICS_COLLECTION_METHOD_NAME,
                 new String[0], null),
  ...
  /**
   * Returns any failure messages stored in the <CODE>failureMessage</CODE>
   * variable.
   *
   ...
   *             false if the name does not match any method.
   */
  @Override()
   ...
   *
   * @return    -1 if the passed method's signiture does not match any stored
