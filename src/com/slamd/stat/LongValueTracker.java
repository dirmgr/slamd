 ...
 * associated with long values.  This is very similar to the integer value
 * tracker, but can deal with much larger values.  This capability does come
 * with a price in terms of performance, so this should only be used for cases
 * in which long values really may be needed.
 *
 *
 * @author   Neil A. Wilson
 */
public class LongValueTracker
       implements StatTracker
{
  ...
  private ArrayList<Long> totalList;
   ...
   */
  public LongValueTracker(String clientID, String threadID, String displayName,
  ...
  {
    return new LongValueTracker(clientID, threadID, displayName,
                                collectionInterval);
  }
   ...
   * this stat tracker.
   *
   * @param  longValue  The value to add to the set of information collected so
   *                    far for this stat tracker.
   */
  public void addValue(long longValue)
  {
  ...
  public void startTracker()
  {
    ...
    if (hasBeenStarted)
    {
      System.err.println("***** WARNING:  Multiple calls to start " +
                         "long value stat tracker " + displayName);
    }
    else
    {
      hasBeenStarted = true;
  ...
  public void stopTracker()
  {
    ...
    if (! hasBeenStarted)
    {
      System.err.println("***** WARNING:  Attempting to stop long value " +
   ...
   *          tracker over each interval.
   */
  public long[] getIntervalTotals()
  {
    long[] totalValues = new long[totalList.size()];
   ...
   */
  public void setIntervalData(long[] intervalTotals, int[] intervalCounts)
  {
    totalValue = 0;
    totalCount = 0;
    maxValue   = Long.MIN_VALUE;
  ...
  public double getAverageValuePerInterval()
  {
    ...
    {
      return (1.0 * totalValue / totalList.size());
    }
    else
    {
      return 0.0;
  ...
  public double getAverageValuePerSecond()
  {
    if (duration > 0)
    {
      return (1.0 * totalValue / duration);
    }
    else
    {
      return 0;
  ...
  public double getStandardDeviation()
  {
    ...
    {
      long intervalTotal = totalList.get(i);
  ...
  public double getHorizontalityTScore()
  {
    ...
    long[] totalArray = getIntervalTotals();
  ...
  public void aggregate(StatTracker[] trackers)
  {
    ...
    long[][] totals = new long[trackers.length][];
    for (int i=0; i < counts.length; i++)
    {
      counts[i] = ((LongValueTracker) trackers[i]).getIntervalCounts();
      totals[i] = ((LongValueTracker) trackers[i]).getIntervalTotals();
      ...
      if (((LongValueTracker) trackers[i]).getMaxValue() > maxValue)
      {
        maxValue = ((LongValueTracker) trackers[i]).getMaxValue();
      }

      if (((LongValueTracker) trackers[i]).getMinValue() < minValue)
      {
        minValue = ((LongValueTracker) trackers[i]).getMinValue();
      }
    }

    int[] aggregateCounts  = new int[min];
    long[] aggregateTotals = new long[min];
    ...
    totalList = new ArrayList<Long>(aggregateCounts.length);
    for (int i=0; i < aggregateCounts.length; i++)
    {
  ...
  {
    return displayName + " -- Total:  " + totalValue +
  ...
  public String getDetailString()
  {
    ...
    returnBuffer.append("Total:  " + totalValue + Constants.EOL);
  ...
  public String getSummaryHTML()
  {
    ...
    html.append("  </TR>" + Constants.EOL);
    html.append("  <TR>" + Constants.EOL);
    html.append("    <TD>" + totalValue + "</TD>" + Constants.EOL);
  ...
  /**
   * Creates a new long value tracker whose contents will be the data elements
   * for this tracker multiplied by the given value.
   *
   * @param  displayName  The display name to use for the resulting tracker.
   * @param  multiplier   The multiplier to use to obtain the new set of data.
   *
   * @return  The created long value tracker.
   */
  public LongValueTracker multiplyValues(String displayName, long multiplier)
  {
    LongValueTracker tracker =
         new LongValueTracker(clientID, threadID, displayName,
                              collectionInterval);

    int[]  intervalCounts = getIntervalCounts();
    long[] intervalTotals = getIntervalTotals();

    for (int i=0; i < intervalTotals.length; i++)
    {
      intervalTotals[i] = intervalTotals[i] * multiplier;
    }

    tracker.setIntervalData(intervalTotals, intervalCounts);
  ...
  /**
   * Creates a new long value tracker whose contents will be the data elements
   * for this tracker divided by the given value.
   *
   * @param  displayName  The display name to use for the resulting tracker.
   * @param  divisor      The divisor to use to obtain the new set of data.
   *
   * @return  The created long value tracker.
   */
  public LongValueTracker divideValues(String displayName, long divisor)
  {
    LongValueTracker tracker =
         new LongValueTracker(clientID, threadID, displayName,
                              collectionInterval);
    ...
    for (int i=0; i < intervalTotals.length; i++)
    {
      intervalTotals[i] = intervalTotals[i] / divisor;
    }
  ...
  public byte[] encode()
  {
    ...
    {
      elements[j] =
           new ASN1OctetString(String.valueOf(totalList.get(i)));
...
//      comparisonTypeParameter,
                                   ...
                                   ParameterList parameters)
  {
    ...
    if (detailLevelStr.equals("Summary Statistics Per Client"))
    {
      ...
      for (int i=0; i < clientIDs.length; i++)
      {
        ...
        {
          LongValueTracker tracker =
               new LongValueTracker(clientIDs[i], "", displayName,
    ...
    else
    {
      ...
      LongValueTracker tracker =
           new LongValueTracker("", "", displayName,
                                trackers[0].getCollectionInterval());
      tracker.aggregate(trackers);
    ...
    {
      LongValueTracker tracker = (LongValueTracker) trackersToGraph[i];

      long[] intervalTotals = tracker.getIntervalTotals();
                                          ...
                                          ParameterList parameters)
  {
    ...
    for (int i=0; i < trackersToGraph.length; i++)
    {
      tracker = (LongValueTracker) trackersToGraph[i];
                                   ...
                                   ParameterList parameters)
  {
    ...
    else
    {
      ...
      for (int i=0; i < jobs.length; i++)
      {
        ...
        LongValueTracker jobTracker =
             new LongValueTracker(null, null, displayName,
