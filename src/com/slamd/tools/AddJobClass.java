...
/**
 * This class defines a utility that may be used to install a job class (already
 * located in the SLAMD server's CLASSPATH) from the command line.  It does this
 * by communicating with the SLAMD administrative interface over HTTP to ensure
 * that all appropriate validity checking is performed, and that any required
...
public class AddJobClass
{
  ...
  {
    AddJobClass addJobClass = new AddJobClass(args);
    if (addJobClass.sendRequest())
    {
      System.out.println("The job class was registered successfully.");
    }
    else
    {
      System.out.println("An error occurred while trying to register the job " +
                         "class.");
    }
  }
   ...
   * @param  args  The command-line arguments provided to this program.
   */
  public AddJobClass(String[] args)
  {
    ...
    // Make sure that the class name was provided.
    if (className == null)
    {
  ...
  public boolean sendRequest()
  {
    ...
    request.addParameter(Constants.SERVLET_PARAM_JOB_CLASS, className);
  ...
  public static void displayUsage()
  {
    ...
    System.out.println(
"USAGE:  java -cp {classpath} AddJobClass {options}" + eol +
"        where {options} include:" + eol +
"-c {className}  -- The fully-qualified job class name" + eol +
