...
/**
 * This class defines a utility that may be used to view information from the
 * SLAMD log file.  It can be used to either retrieve the entire log file or the
 * last <I>N</I> lines.
 *
 *
...
public class ViewLog
{
  ...
  {
    ViewLog viewLog = new ViewLog(args);
    if (! viewLog.sendRequest())
    {
   ...
   *
   * @return  {@code true} if the job was enabled properly, or
   *          {@code false} if not.
   */
  public boolean sendRequest()
  {
    ...
    if (showAll)
    {
      request.addParameter(Constants.SERVLET_PARAM_LOG_VIEW_ALL, "on");
    }
    else
    {
      request.addParameter(Constants.SERVLET_PARAM_LOG_VIEW_LINES,
    ...
    try
    {
      ...
      while (line != null)
      {
        if (line.equals("<!-- START LOG OUTPUT -->"))
        {
          printing = true;
        }
        else if (line.equals("<!-- END LOG OUTPUT -->"))
        {
          printing = false;
        }
        else if (printing)
        {
          int brPos = line.toLowerCase().indexOf("<br>");
          if (brPos >= 0)
          {
            System.out.println(line.substring(0, brPos));
          }
          else
          {
  ...
  public static void displayUsage()
  {
    ...
    System.out.println(
"USAGE:  java -cp {classpath} ViewLog {options}" + eol +
"        where {options} include:" + eol +
"-n {numLines}   -- The number of lines to show from the end of the log" + eol +
"-a              -- Show the entire log file" + eol +
