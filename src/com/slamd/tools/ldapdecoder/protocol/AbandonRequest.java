...
/**
 * This class defines an LDAP abandon request, which is used to abandon a
 * previously requested operation in a directory server.
 *
 *
       ...
       extends ProtocolOp
{
   ...
   */
  public AbandonRequest(int idToAbandon)
  {
  ...
  {
    return new ASN1Integer(ABANDON_REQUEST_TYPE, idToAbandon);
  }
   ...
   *
   * @return  The decoded abandon request.
   *
   * @throws  ProtocolException  If a problem occurs while decoding the provided
   *                             ASN.1 element as an abandon request.
   */
  public static AbandonRequest decodeAbandonRequest(ASN1Element element)
         throws ProtocolException
  {
    try
    {
      int idToAbandon = element.decodeAsInteger().getIntValue();
      return new AbandonRequest(idToAbandon);
    }
    catch (Exception e)
    {
      throw new ProtocolException("Unable to decode abandon request", e);
    }
  }
  ...
  public String toString(int indent)
  {
    ...
    buffer.append(indentBuf).append("Message ID of Operation to Abandon: ").
           append(idToAbandon).append(LDAPMessage.EOL);
  ...
  {
    scriptWriter.println("#### Abandon request captured at " + new Date());
